  ...
  modules: [
    {
      ...
      params: [
        ...
        { 
          ...
          type: "enumeration", 
          enumOptions: [
            { title: "全部地区", value: "" }, { title: "中国", value: "CN" }, { title: "美国", value: "US" }, { title: "韩国", value: "KR" }, { title: "日本", value: "JP" }, { title: "英国", value: "GB" }
          ], 
          value: "" 
        },
        {
    ...
    },
    {
      ...
      params: [
        { name: "language", title: "语言", type: "language", value: "zh-CN" },
        {
          ...
          value: "213",
          enumOptions: [
            { title: "Netflix", value: "213" }, { title: "Disney+", value: "2739" }, { title: "Apple TV+", value: "2552" }
          ]
        },
        {
          name: "sort_by",
...
async function loadTodayHotTV(params = {}) {
    ...
    if (!with_origin_country) {
        // 使用你的 forward-eta 域名 + 时间戳
        const url = `https://forward-eta.vercel.app/data/TMDB_Trending.json?t=${Date.now()}`;
        const res = await Widget.http.get(url);
        return res.data.results.filter(i => i.media_type === "tv" || !i.media_type);
    }
    return await fetchTmdbDiscover("tv", { language, with_origin_country, sort_by, page });
}
...
async function loadTodayHotMovies(params = {}) {
    ...
    return await fetchTmdbDiscover("movie", { language, with_origin_country, sort_by, page });
}

async function fetchTmdbDiscover(type, p) {
    const apiKey = "63d7e5d8935c754b5dfd9744c6888c3e";
    const url = `https://api.themoviedb.org/3/discover/${type}?api_key=${apiKey}&language=${p.language}&with_origin_country=${p.with_origin_country}&sort_by=${p.sort_by}&page=${p.page}`;
    const res = await Widget.http.get(url);
